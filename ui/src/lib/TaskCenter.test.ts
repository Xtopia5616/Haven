...
const commonProps = {
	...
	sessionTitleFor: ({ sessionId }: { sessionId?: string }) => sessionId === 'ses-1' ? '研究会话' : '',
	actionDuration: () => '3s',
	...
	it('opens task detail and routes source-session actions', async () => {
		...
		render(TaskCenter, {
			...
			runningBackgroundActions: [
				{ id: 'act-1', kind: 'background', command: '整理下载目录', status: 'running', sessionId: 'ses-1', startedAt: '2026-09-03T10:00:00Z' },
			],
			onOpenSession,
			onCancel,
		});

		expect(screen.getByRole('heading', { name: '整理下载目录' })).toBeTruthy();
