			...
			{:else if toolName === 'agent'}
				...
				{:else if Array.isArray(data.agents)}
					...
					{#if data.agents.length > 0}
						<div class="tool-card-list">
							{#each data.agents as a (a.name ?? a.session_id ?? a.agent)}
								<div class="action-row">
									<span class="action-id">{a.title || a.name || a.session_id || a.agent}</span>
									{#if a.role}<span class="scheduled-mode">{a.role}</span>{/if}
									{#if a.status}<span class="status-badge status-{a.status === 'online' ? 'completed' : 'cancelled'}">{a.status}</span>{/if}
								</div>
							{/each}
						</div>
					{:else}
