...
- Serialize actor-registry and durable-session lifecycle operations with a
  ...
  use the same gate; direct runs re-check actor membership under the gate after
...
propagation, same-session FIFO, independent-session parallel submission, and
draft-lane ordering. Run the workspace Rust tests and the UI check/test gates
before release.
