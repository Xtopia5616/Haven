...
2. OpenAI Chat 和 Anthropic 在 adapter 边界做防御性清洗：确保根值是对象，
   ...
   `oneOf`/`anyOf` 等语义。
3. OpenAI Responses 的 function tool 明确发送 `strict: false`，并发送清洗后
   的 schema，避免服务端把完整 schema 自动收窄到 strict 子集；Haven 继续在
   执行前做严格校验。
...
工具执行仍由 Haven 的完整 schema 校验。未知 OpenAI-compatible 网关仍是一个
明确的审查项：当前代码不会误把它识别成 xAI 或 Gemini，但也不能保证它接受
根级 union。
