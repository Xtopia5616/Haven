...
2. operation view 从 `haven-tools::prompts` 的集中目录取得 `ToolPrompt`，并给窄
   schema 补充 `title` 与 `description`；原始分支校验、固定 discriminator、授权和
   执行路径不变。
3. Agent 系统 Prompt 将内置工具按 family 聚合为紧凑的 `use / avoid / ops` 目录行。
   完整名称、参数和实时可用性仍以每一步的 `tools[]` 为唯一权威；目录内容会清洗
   控制字符并限制长度。
