...
可能把同一份图片/音频 bytes 同时写入 `MessageAttachment`、`messages.attachments`
...
1. `messages.media_inputs` 是消息的 provider-neutral 媒体持久化投影，包含资产元数据、
   表示、provenance、availability 和 preferred representation。`messages.attachments`
   保留为 UI/旧 IPC 的兼容投影，不再保存 base64；受管路径只在可信 host 根目录内为
   历史预览重新读取 bytes。
2. `TranscriptRecord::UserInject` 新写入 `media_inputs`，旧 `attachments` 只读兼容。
   旧快照加载时立即转换并清空旧字段；之后的 snapshot 不会再序列化 inline bytes。
   `CompactSummary` 也在序列化边界把 inline image/audio 替换为带 MIME 的安全标记。
3. OCR/STT 成功会在同一 `MessageAttachment` 兼容投影上追加持久化的派生
   `MediaRepresentation`，设置 preferred kind，并保留原始附件以供 retry、resume 和
   其他 provider 重新规划。正文中的 provenance fence 只作为当前 UI/旧消息的展示投影。
...
5. 数据库 schema 从 v16 提升到 v17。项目当前不做运行时迁移，旧数据库按发布说明重置。
