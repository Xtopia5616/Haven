...
| `confirm:requested` | `ConfirmationRequestedEvent { step_id, tool_name, risk_level, session_id, params, permission_key }` | 聊天页 | 必须先由后端 SafetyGateway 创建；`params` 是确认所需动态扩展点，决策仍由后端校验。 |
