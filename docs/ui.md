...
2. **上下文连续**：会话、工具调用、确认、后台任务和错误都要在同一条可追溯的上下文中呈现。
...
| 问题 | 用户影响 | 重构方向 |
|---|---|---|
| 顶部导航、状态 chip、状态菜单、任务入口和页面内容分散表达 | 用户无法快速判断“我在哪”和“现在发生了什么” | 建立工作区壳层，拆分导航、会话状态和任务中心职责 |
...
- “任务中心”是前台运行、后台任务、定时任务和已完成任务的统一入口。状态菜单只保留为轻量摘要，不再承担完整任务管理。
- 对话中的工具调用、确认和后台任务必须保留返回来源；用户从任务中心打开时，应能回到触发它的会话。
- 工具页采用列表/详情布局：左侧或上方是可搜索、可筛选列表，右侧或抽屉是详情与操作；不得为每个项目复制一套卡片操作逻辑。
- 记忆页按“会话历史、长期记忆、维护/检索”分组，避免把历史记录和记忆条目混在同一层级。
...
2. 后台或定时任务显示在任务中心，并在会话中保留紧凑的来源和进度入口。
...
| 后台 / 定时任务 | 来源、进度、状态、下一步 | 能从会话进入任务详情，也能从任务详情回到来源会话；取消后有明确结果 |
...
| `TaskCenter` / `TaskDrawer` | 前台、后台、定时任务的统一展示和操作 | 修改消息时间线内部数据 |
...
| Phase 3：任务中心 | 前台、后台、定时和已完成任务的统一列表/详情 | 任务来源、生命周期、取消和回到会话路径清晰；全局状态摘要不再替代任务中心 |
...
- 设置、工具、记忆和任务中心使用相同的页面层级、列表/详情模式、状态反馈和动作语义。
...
| Phase 3 | `d226359` | 任务中心列表/详情、取消与回到会话；check/test/build 通过 |
