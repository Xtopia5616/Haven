...
- **edges / items**（prompt 仍称 USER FACTS / Past excerpts）= **跨会话**检索；同会话不进 Past excerpts（`exclude_session_id`）
- **DB messages** = 持久化 + 抽取源；**不**并进记忆索引；Additional context 不与首条 user 重复
...
**计数**：待办 **0** · 可选 **7**（X 史诗/产品）· 不推荐 **6** · 完成归档本轮 **14**（M1–M6、R1–R6、X7、X1）· 史诗计入可选。
