...
> 状态：P0–P2 基线已完成；平台适配与战略性后续重构进行中
> 制定日期：2026-08-26  
...
> 当前 IPC 目录（2026-09-14）已扩展为 70 个命令和 40 个事件；上面的完成记录保留各自完成当日的历史数量，不代表当前目录规模。
...
- 2026-09-14：P0 人工交互生命周期收口：ask、confirm、scheduled confirm 统一为 `InteractionRequest`，session 只保留通用 `Paused`，统一事件和前端 `interactionStore` 负责投影、恢复与解析（ADR 0156）。
...
- 2026-09-01：P2 Agent 上下文与工具调度三次收口：turn-start 统一收集本地队列/inbox，turn-end 只补收本地输入；单工具与并行工具共用 plan、取消修复和 ordered projection（ADR 0064）。
- 2026-09-01：P2 Memory 召回边界收口：`MemoryQuery` / `MemoryRecall` / `MemoryRetriever` 统一关键词、向量、敏感过滤与 prompt cache，embedding provider 只负责获取向量（ADR 0063）。
...
- 2026-09-15：P1 Tools 管理面收口：模型目录保留五个 capability-scoped admin tools，五个 surface 均由 `TypedToolOperation` 实现；native Tauri confirmation queue 改存 typed admin request，并删除旧 broad dispatcher、参数和操作类型（ADR 0070、0071）。

- 2026-09-02：P1 Tools 契约首条迁移：haven_config 使用 TypedToolOperation，由 typed operation 统一提供 args/output/error 与风险、幂等、取消/超时和并发 metadata（ADR 0071）。
...
- 2026-09-19：P1 恢复持久化补齐 append-only 两阶段标记、scratch 保留和严格 branch cutoff 失败边界；数据库故障注入覆盖恢复消息写入失败（ADR 0175）。
