...
- 2026-08-31：P2 Agent 请求与流式代次收口：以 `RequestContext` 统一 provider 请求投影；inbox envelope 保留独立边界；thought/reasoning 共用有序 chunk 队列，并以 `agent:stream_reset` 隔离 failover/retry 输出；`PartialStore` 以 generation 丢弃晚到 checkpoint；前端只合并相邻 chunk（ADR 0058）。
