...
- 2026-09-10：P1/P2 Tools/Agent/Memory/UI 多模态用量边界收口：媒体派生文本只保留一个 `media.content`，模型观察去除运行时元数据；媒体 LLM 调用单独记录为 `call_kind=media`，不降低 Agent 主循环缓存率，UI 分组展示媒体推理用量（ADR 0124）。
